import { listSkills, useSkill } from "../skills.ts";
import { defineTool, type ToolDefinition } from "./types.ts";

export const SKILL_TOOLS = {
    use_skill: defineTool(
        "use_skill",
        "Load a skill by name from workspace/skills/<SKILL>.md. Use this before applying a skill's instructions.",
        {
            name: {
                type: "string",
                description: "Skill filename under workspace/skills.",
            },
        },
        ["name"],
        {
            handler: async (args) => {
                ...
                return await useSkill(String(args.name));
            },
        },
    ),
